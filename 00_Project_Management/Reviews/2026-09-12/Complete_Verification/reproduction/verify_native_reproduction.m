function summary=verify_native_reproduction(outputFolder,caseSelection)
...
mkdir(outputFolder);
root='C:\Users\adand\OneDrive\Desktop\Projects\EMI-Resilient Control System for Robotics';
 ...
 for cp=[30 300]
  for initialA=[0 1]
   ...
   for maxStep=[.5 .25 .125]*1e-9
    ...
    % Reproduce the final archived settings without retuning any case.
    archived=fullfile(root,'06_Circuit_Simulations','FOUR_WAY','results','native_checkpoint_20260911');
    sourceSettings="";
    for archiveName=["fourway_native_acceptance_v1","fourway_native_tightened_v2"]
     candidate=fullfile(archived,archiveName,caseName,sprintf('native_%gns',maxStep*1e9),'native_raw.mat');
     if isfile(candidate),sourceSettings=candidate;end
    end
    assert(sourceSettings~="",'VERIFY:MissingSettings','Missing final archived settings.');
    loadedSettings=load(sourceSettings,'settings');settings=loadedSettings.settings;
    assert(settings.maxStep_s==maxStep && settings.stopTime_s==50e-6,'VERIFY:SettingsMismatch');
    fid=fopen(fullfile(runFolder,'settings_provenance.json'),'w');
    fprintf(fid,'%s',jsonencode(struct('source',sourceSettings,'settings',settings),PrettyPrint=true));fclose(fid);
    started=tic;
    try
