...
The original suite verifies causal/monotonic packet acceptance and hold-last behavior. The subsequent Phase 2 completion campaign adds seven hand-constructed schedules for collisions, stale arrivals, loss and record boundaries, with exact frozen-profile regression. The 2026-09-09 sensitivity extension adds independent nonzero scaling tests for the shared resistance, shared inductance, and ground-conversion terms, plus inactive/diagnostic parameter checks; its then-current 45-test count is historical. The current active suite passes 174/174 tests. See the [completion report](../03_MATLAB/results/development/phase2_completion/Phase2_Completion_Report.md).
