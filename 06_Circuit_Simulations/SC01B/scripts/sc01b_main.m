...
for name=caseNames
    ...
    for k=1:numel(spice)
        ...
        runFolder=fullfile(caseFolder,tag);r=simulate_sc01b_ngspice(pp,runFolder,options.NgspiceExecutable);spice{k}=r;
        runRows{end+1}=runRow(name,tag,r,r.runtime_s);
        ...
        fprintf('SC01B COMPLETE %s %s samples %d elapsed %.1fs\n',name,tag,numel(r.time_s),r.runtime_s);
    end
    ...
    pairs={native{1},native{3},"native_0.5_to_0.125ns"; ...
        native{2},native{3},"native_0.25_to_0.125ns"; ...
        spice{1},spice{2},"spice_0.25_to_0.125ns"; ...
        native{3},spice{2},"native_to_original_spice_0.125ns"};
    if ismember(name,c.consistencyCases)
        ...
        try
            r=simulate_sc01b_ngspice(pp,runFolder,options.NgspiceExecutable);
            runRows{end+1}=runRow(name,"spice_tight_tolerance",r,r.runtime_s);
            duplicate=fullfile(runFolder,'waveform.dat');if isfile(duplicate),delete(duplicate);end
