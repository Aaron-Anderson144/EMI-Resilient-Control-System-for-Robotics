...
model='EMI_SC01B_Device_Halfbridge';modelFile=fullfile(root,'models',[model '.slx']);
if ~bdIsLoaded(model) && exist(modelFile,'file'),load_system(modelFile);end
rebuild=~bdIsLoaded(model);
if ~rebuild
    ws=get_param(model,'ModelWorkspace');rebuild=~hasVariable(ws,'sc01bSchema') || getVariable(ws,'sc01bSchema')~=3;
end
