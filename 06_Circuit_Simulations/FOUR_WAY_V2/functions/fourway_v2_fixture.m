function fixture=fourway_v2_fixture(id,armId,exposed,variantId,closure_s)
%FOURWAY_FIXTURE Materialize PLAN-V2 without changing historical defaults.
root=fourway_v2_root();
c=jsondecode(fileread(fullfile(root,'04_EMI_Models','four_way_emi_v2_configuration.json')));
matrix=readtable(fullfile(root,'04_EMI_Models','four_way_emi_v2_fixtures.csv'),'TextType','string');
if nargin<5,closure_s=c.source.synthetic_return_s;end
row=matrix(matrix.fixture_id==string(id),:);
...
assert(any(closure_s==c.source_closure_diagnostics.return_duration_s),'EMIProject:FourWayClosure','Return duration is not declared.');
...
assert(~any(profiles.sourceFault)&&all(profiles.communication.sampleReceived)&& ...
 ...
 'EMIProject:FourWayIsolation','Unrelated fault or packet scheduling entered PLAN-V2.');
fixture=struct('id',string(id),'partition',row.stage,'arm',string(armId), ...
 ...
 'closure_s',closure_s,'variant',fourway_v2_variant(variantId),'design',c,'root',string(root), ...
